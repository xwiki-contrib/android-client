...
public class XMdlToSmplConverter {
	 ...
	 * 
	 * @param parts
	 *            {[PAGE]?|[OBJECTS]?|[COMMENTS]?|[TAGS]?} or ALL Ex1: make
	 *            simple xml page + simple xml objects (PAGE+OBJECTS) or
	 *            (PAGE|OBJECTS) Ex2: make Simplexml Attachment objects +
	 *            comments + objects (first class entities derived from Object
	 *            objects) (ATTACHMENTS+COMMENTS+OBJECTS)
	 */
	...
	public SimpleXmlDocpad convertDocument(SimpleDocument doc) {
		...
		if ((parts & OBJECTS) == OBJECTS) {
			...
			for (Entry<String, XSimpleObject> en : entrySet) {
				pad.editedObjects.put(en.getKey(),
						toSimpleXmlObj(en.getValue()));
			}
		...
		if ((parts & COMMENTS) == COMMENTS) {
			throw new UnsupportedOperationException(
					"converting to first class entities not supported");
		}
		if ((parts & TAGS) == TAGS) {
			throw new UnsupportedOperationException(
					"converting to first class entities not supported");
	...
	private org.xwiki.android.resources.Object toSimpleXmlObj(XSimpleObject xobj) {
		...
		for (Entry<String, XProperty> entry : entrySet) {
			...
			p.setValue(xp.getValue().toString());
