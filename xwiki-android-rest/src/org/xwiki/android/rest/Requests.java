...
public class Requests {
	...
	public SearchResults requestSearch(String wikiName, String spaceName,
			String keyword) {
	...
	public Pages requestAllPages(String wikiName, String spaceName) {
		wikiName = convertToUTF(wikiName);
		spaceName = convertToUTF(spaceName);
		PageResources pagesresources = new PageResources(URLprefix, wikiName,
				spaceName);
		if(isAuthenticated){
	...
	// calendar]
	public History requestPageHistory(String wikiName, String spaceName,
			String pageName) {
		wikiName = convertToUTF(wikiName);
		spaceName = convertToUTF(spaceName);
		pageName = convertToUTF(pageName);

		HistoryResources historyresources = new HistoryResources(URLprefix,
	...
	// Method for getting page history [complete but not working due to
	// calendar]
	public Page requestPageInVersionHistory(String wikiName, String spaceName,
	...
	public Pages requestPageChildren(String wikiName, String spaceName,
			String pageName) {
		wikiName = convertToUTF(wikiName);
		spaceName = convertToUTF(spaceName);
		pageName = convertToUTF(pageName);
	...
	public Tags requestPageTags(String wikiName, String spaceName,
			String pageName) {
		wikiName = convertToUTF(wikiName);
		spaceName = convertToUTF(spaceName);
		pageName = convertToUTF(pageName);
	...
	// calendar]
	public Comments requestPageComments(String wikiName, String spaceName,
			String pageName) {
		wikiName = convertToUTF(wikiName);
		spaceName = convertToUTF(spaceName);
		pageName = convertToUTF(pageName);

		CommentResources commentresources = new CommentResources(URLprefix,
				wikiName, spaceName, pageName);
		if(isAuthenticated){
	...
	public Comment requestPageComments(String wikiName, String spaceName,
			String pageName, String commentId) {
		wikiName = convertToUTF(wikiName);
		spaceName = convertToUTF(spaceName);
		pageName = convertToUTF(pageName);
	...
	// Method for getting page comments with comment id [complete but not
	// working due to calendar]
	public Comments requestPageCommentsInHistory(String wikiName,
	...
	// due to calendar]
	public Attachments requestAllPageAttachments(String wikiName,
		...
		AttachmentResources attachmentresources = new AttachmentResources(
				URLprefix, wikiName, spaceName, pageName);
		if(isAuthenticated){
	...
	// bytes]
	public String requestPageAttachment(String wikiName, String spaceName,
	...
	public Attachments requestPageAttachmentsInHistory(String wikiName,
			String spaceName, String pageName, String pageVersion) {
		wikiName = convertToUTF(wikiName);
		spaceName = convertToUTF(spaceName);
		pageName = convertToUTF(pageName);
	...
	public String requestPageAttachmentsInHistory(String wikiName,
			String spaceName, String pageName, String pageVersion,
	...
	// working due to calendar]
	public Attachments requestPageAttachmentsInAttachmentHistory(
		...
		return attachmentresources
				.getPageAttachmentsInAttachmentHistory(attachmentName);
	}

	// Method for getting attachment by name in a specific attachment version
	// [should return attachment bytes]
	public String requestPageAttachmentsInAttachmentHistory(String wikiName,
		...
		return attachmentresources.getPageAttachmentsInAttachmentHistory(
				attachmentName, attachmentVersion);
	}
	...
	public Translations requestAllPageTranslations(String wikiName,
			String spaceName, String pageName) {
		wikiName = convertToUTF(wikiName);
		spaceName = convertToUTF(spaceName);
		pageName = convertToUTF(pageName);

		TranslationResources translationresources = new TranslationResources(
				URLprefix, wikiName, spaceName, pageName);
		if(isAuthenticated){
	...
	public Page requestPageTranslation(String wikiName, String spaceName,
			String pageName, String language) {
		wikiName = convertToUTF(wikiName);
		spaceName = convertToUTF(spaceName);
		pageName = convertToUTF(pageName);

		PageResources pageresources = new PageResources(URLprefix, wikiName,
				spaceName);
		if(isAuthenticated){
	...
	public History requestPageHistoryInLanguage(String wikiName,
	...
	public Page requestPageTranslationInHistory(String wikiName,
	...
	public Objects requestAllObjects(String wikiName, String spaceName,
			String pageName) {
		wikiName = convertToUTF(wikiName);
		spaceName = convertToUTF(spaceName);
		pageName = convertToUTF(pageName);

		ObjectResources objectresources = new ObjectResources(URLprefix,
				wikiName, spaceName, pageName);
		if(isAuthenticated){
	...
	public Objects requestObjectsInClass(String wikiName, String spaceName,
			String pageName, String objectClassname) {
		wikiName = convertToUTF(wikiName);
		spaceName = convertToUTF(spaceName);
		pageName = convertToUTF(pageName);
	...
	public Object requestObject(String wikiName, String spaceName,
	...
	// Method for getting object in specific class in a page
	public Properties requestObjectAllProperties(String wikiName,
		...
		return objectresources.getObjectProperties(objectClassname,
				objectNumber);
	}

	// Method for getting object in specific class in a page
	public Property requestObjectProperty(String wikiName, String spaceName,
			String pageName, String objectClassname, String objectNumber,
			String propertyName) {
		wikiName = convertToUTF(wikiName);
		spaceName = convertToUTF(spaceName);
		...
		return objectresources.getObjectProperty(objectClassname, objectNumber,
				propertyName);
	}
